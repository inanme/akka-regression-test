...
object BasicTransformation extends App with MyResources {
  ...
  Source
    .fromIterator(() => text.split("\\s").iterator)
    .map(_.toUpperCase)
    .runForeach(println)
...
object Tick extends App with MyResources {
  ...
  val buffer =
    ...
    ) // back-pressures the source if the buffer is full
...
object Kros extends App with MyResources {
  ...
  val x1: Sink[String, NotUsed]                     = fb.toMat(sink)(Keep.left)
  val x2: Sink[String, Future[IOResult]]            = fb.toMat(sink)(Keep.right)
  val x3: Sink[String, (NotUsed, Future[IOResult])] = fb.toMat(sink)(Keep.both)
  val fs =
    so.map(_ + 1)
      .map(_.toString)
      .runWith(x2)
      .onComplete(_ => system.terminate()) //toMat(sink)(Keep.right).run()
